...
// Leaderboard
app.get('/api/leaderboard/gifts', async (_req, res) => {
...
io.use((socket, next) => {
    ...
    if (!token)
        return next(); // allow anonymous read
    try {
        const payload = jwt.verify(token, JWT_SECRET);
        socket.user = { id: payload.id, handle: payload.handle };
    }
...
giftsNs.on('connection', (socket) => {
    ...
    socket.on('gift:send', async (payload) => {
        ...
        if (!user)
            return socket.emit('error', { error: 'auth required for gifts' });
        if (!payload?.code)
            return socket.emit('error', { error: 'missing code' });
        const gift = await Gift.findOne({ code: payload.code });
        if (!gift)
            return socket.emit('error', { error: 'gift not found' });
        const dbUser = await User.findById(user.id);
        if (!dbUser)
            return socket.emit('error', { error: 'user not found' });
        if (dbUser.tokens < gift.tokenCost)
            return socket.emit('error', { error: 'insufficient tokens' });
        ...
        await redis.zincrby(LEADERBOARD_KEY, gift.tokenCost, dbUser._id.toString());
        const event = { code: gift.code, from: dbUser.handle, tokens: gift.tokenCost, commandAction: gift.commandAction };
